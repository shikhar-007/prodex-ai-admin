...
function AppLayout({ children }) {
  ...
  return (
    <div className="w-full flex">
      ...
      <div
        ref={divRef}
        className={`md:block ${openMenu ? "visible transition-all h-screen" : "hidden"
          } fixed md:static z-50`}
      >
        <SideBar openMenu={openMenu} setOpenMenu={setOpenMenu} />
      </div>
      <div
        className={`min-h-screen w-full flex flex-col bg-appLayoutBg ${openMenu ? "blur-[8px] md:blur-0" : ""
          } bg-cover `}
      >
